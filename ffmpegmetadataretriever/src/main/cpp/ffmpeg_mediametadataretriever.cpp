...
void init(State **ps) {
    ...
    if (!state) {
        state = static_cast<State *>(av_mallocz(sizeof(State)));
    }
    ...
    *got_packet_ptr = 0;
    ...
    if (ret >= 0 && state->native_window) {
        ...
        if (ANativeWindow_lock(state->native_window, &windowBuffer, NULL) == 0) {
            ...
            for (h = 0; h < height; h++)  {
                memcpy(((int *)windowBuffer.bits) + h * windowBuffer.stride * 4,
                       ((int *)buffer) + h * frame->linesize[0],
                       width*4);
            }
...
int get_scaled_frame_at_time(State **ps, int64_t timeUs, int option, AVPacket *pkt, int width, int height) {
    ...
    Options opt = static_cast<Options>(option);
